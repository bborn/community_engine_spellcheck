...
AtD_proofread_click_count++;plugin.editor.setProgressState(1);plugin._removeWords();plugin.sendRequest('checkDocument',ed.getContent({format:'raw'}),function(request)
...
{dom.remove(n,1);}});});editor.onSubmit.add(function(ed,e){autocheck=editor.getParam("atd_autocheck",true);if(!autocheck){return true;}
...
{var id=t.editor.getParam("atd_rpc_id","12345678");tinymce.util.XHR.send({url:url+encodeURI(e.target.innerHTML).replace(/&/g,'%26')+"&key="+id,content_type:'application/x-www-form-urlencoded',async:true,type:'GET',success:function(type,req,o)
...
new Ajax.Request(url+"/"+file,{onSuccess:success,postBody:"data="+encodeURI(data).replace(/&/g,'%26')+"&key="+id,onFailure:function(res){plugin.editor.setProgressState(0);alert(res.request.method+"\n"+res.status+"\nAt: "+res.request.url);}});}});tinymce.PluginManager.add('AtD',tinymce.plugins.AfterTheDeadlinePlugin);})();