...
(function() 
{
	 ...
	 core.findSpans = function(parent) 
      ...
      init : function(ed, url) 
      {
         ...
         editor.addCommand('mceWritingImprovementTool', function(callback) 
         {
            ...
            plugin.sendRequest('checkDocument', ed.getContent({ format: 'raw' }), function(request)
            {
   ...
   editor.onSubmit.add(function(ed, e) {
     autocheck = this.editor.getParam("atd_autocheck",  true);
     if(!autocheck){
       return true;
     }
     if(plugin.force_save){
       ...
       Event.stop(e);          
       ed.execCommand('mceWritingImprovementTool', function(count){
         if (count > 0){
           ed.windowManager.confirm("There are " + count + " spelling errors. Do you want to continue saving?", function(s){
             if(s){
               e.target.submit();
             } 
           });
	 ...
	 editor.onBeforeExecCommand.add(function(editor, command) 
      ...
      sendRequest : function(file, data, success)
      {
         ...
         new Ajax.Request(url + "/" + file, {
           onSuccess: success,
           postBody : "data=" + encodeURI(data).replace(/&/g, '%26') + "&key=" + id, 
           onFailure  : function( res ){
              plugin.editor.setProgressState(0);
              alert( res.request.method + "\n" + res.status + "\nAt: " + res.request.url ); 
           }
         });
/*         tinymce.util.XHR.send({
            url          : url + "/" + file,
